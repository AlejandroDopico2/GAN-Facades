...
class SemanticSegmenter(FacadesModel):
    ...
    def train(
        ...
        **kwargs,
        ...
        points = torch.cat(
            [train[i][1].flatten(1, 2) for i in range(self.N_KMEANS)], 1
        ).permute(1, 0)
        _, self.centers = kmeans(
            X=points,
    ...
    def segment(self, x: torch.Tensor) -> torch.Tensor:
        with contextlib.redirect_stdout(None):
            ...
            for img in x.unbind(0):
                labs = kmeans_predict(
                    img.flatten(1, 2).T, self.centers, "euclidean", device=self.device
                )
                segment = (
                    self.centers[labs].reshape(*img.shape[-2:], -1).permute(2, 0, 1)
                )
