...
class Metric:
    ...
    def __repr__(self) -> str:
        return ", ".join(
            f"{metric}={getattr(self, metric)*(100 if metric in self.SCALE else 1):.2f}"
    ...
    def improves(self, other: Metric) -> bool:
        if self.MODE == "max":
            return any(
                getattr(self, metric) > getattr(other, metric) for metric in self.KEY
            )
        else:
            return any(
                getattr(self, metric) < getattr(other, metric) for metric in self.KEY
...
class SegmentationMetric(Metric):
    ...
    def __call__(self, preds: torch.Tensor, golds: torch.Tensor) -> SegmentationMetric:
        ...
        # true positives
        tps = torch.tensor(
            [((golds == label) & (preds == label)).sum() for label in labels]
        ).to(preds.device)
        # predicted positives
        pps = torch.tensor([(preds == label).sum() for label in labels]).to(
            preds.device
        )
...
class GenerationMetric(SegmentationMetric):
    ...
    def __call__(
        ...
        for attr in real_metric.ATTRIBUTES:
            self.__setattr__(
                f"real_{attr}",
                getattr(self, f"real_{attr}") + getattr(real_metric, attr),
            )
        for attr in fake_metric.ATTRIBUTES:
            self.__setattr__(
                f"fake_{attr}",
                getattr(self, f"fake_{attr}") + getattr(fake_metric, attr),
            )
        self.n += 1
