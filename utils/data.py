...
class FacadesDataset(Dataset):
    ...
    def split(
        self, *sizes: List[Union[int, float]], shuffle: bool = True, seed: int = 42
    ) -> Tuple[FacadesDataset]:
        random.seed(seed)
        if shuffle:
            random.shuffle(self.image_paths)
        lens = (
            [int(len(self) * ratio) for ratio in sizes]
        ...
        splits = [
            FacadesDataset(
                self.image_paths[indices[i] : indices[i + 1]], self.transform
            )
