...
class ConvBlock(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if n_layers == 1:
            self.conv = nn.Conv2d(
                in_channels=n_in, out_channels=n_out or n_in, kernel_size=k, **kwargs
            )
        else:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.offset_conv = nn.Conv2d(
            n_in, 2 * k * k, kernel_size=k, padding=padding, stride=stride, **kwargs
        )
        self.modulator_conv = nn.Conv2d(
            n_in, 1 * k * k, kernel_size=k, padding=padding, stride=stride, **kwargs
        )
        self.regular_conv = nn.Conv2d(
            n_in, n_out or n_in, k, padding=padding, stride=stride, **kwargs
        )
        self.bn = nn.BatchNorm2d(n_out or n_in)
        self.act = act
        self.drop = nn.Dropout2d(dropout, inplace=True)
