...
from modules.attnunet.utils import inconv, down_block
from modules.attnunet.utils import get_block
from modules.attnunet.modules import attention_up_block

class AttentionUNet(nn.Module):

    def __init__(self, input_channels: int, output_channels: int, base_ch=64, block='BasicBlock', pool=True):
        super().__init__()

        num_block = 2
        block = get_block(block)

        self.inc = inconv(input_channels, base_ch, block=block)
        ...
        self.outc = nn.Sequential(nn.Conv2d(base_ch, output_channels, kernel_size=1), nn.Sigmoid())
