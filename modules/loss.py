...
class GeneratorLoss(nn.Module):
    def __init__(self, alpha=1, weights: bool = False, k1: int = 3, k2: int = 5):
        ...
        if weights:
            self.mask = (
                gaussian2d(256, 256).unsqueeze(0).unsqueeze(0).repeat(1, 1, 1, 1)
            )
        self.k1, self.k2 = k1, k2

    def forward(
        self, fakes: torch.Tensor, reals: torch.Tensor, fake_preds: torch.Tensor
    ):
        ...
        if self.weights:
            ...
            W = max_pool2d(
                gray_reals, kernel_size=self.k1, stride=1, padding=self.k1 // 2
            ) + max_pool2d(
                -gray_reals, kernel_size=self.k1, stride=1, padding=self.k1 // 2
            )
